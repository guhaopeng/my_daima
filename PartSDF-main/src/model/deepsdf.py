...
class DeepSDF(nn.Module):
    ...
    def __init__(self, latent_dim=256, hidden_dim=512, n_layers=8, in_insert=[4],
        ...
        """
        Args:
            latent_dim (int): dimension of the latent code. # 256 latent的维度
            hidden_dim (int): dimension of hidden layers. # 512 隐藏层的维度
            n_layers (int): number of layers. # 8 层
            in_insert (list of int): list of layers where to reinsert the input. # 4 层插入输入
    ...
    def forward(self, lat, xyz, **kwargs):
        """
        Args:

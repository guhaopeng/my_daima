...
def compute_sdf(model, latent, xyz, return_parts=False, max_batch=32**3, verbose=False, device=get_device(), **model_kwargs):
    ...
    **model_kwargs: (optional)
    ...
    query_cond = model_kwargs.pop("query_cond", None)
    ...
    for i in range(0, n_points, max_batch):
        ...
        if query_cond is not None:
            cond = torch.as_tensor(query_cond, dtype=xyz_subset.dtype, device=device)
            if cond.ndim == 0:
                cond = cond.view(1)
            cond = cond.view(*([1] * (xyz_subset.ndim - 1)), -1).expand(xyz_subset.shape[:-1] + (-1,))
            xyz_subset = torch.cat([xyz_subset, cond], dim=-1)
        if return_parts:

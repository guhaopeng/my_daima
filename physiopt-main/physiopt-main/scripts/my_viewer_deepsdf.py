...
try:
    from pytorch.mma import kktcheck, mmasub
...
class ViewerSdf(ViewerBase):
    ...
    def init_optimizer(self):
        if not HAS_MMA:
            raise ImportError("Cannot import `pytorch.mma.mmasub`.")
    ...
    def _init_mma_state(self) -> None:
        ...
        self.mma_c = 1e3 * torch.ones((self.mma_m, 1), dtype=dtype, device=device)
    ...
    def _build_mma_bounds_(
        ...
        xmin = -MMA_BOUND * torch.ones((self.mma_n, 1), dtype=dtype, device=device)
        xmax = MMA_BOUND * torch.ones((self.mma_n, 1), dtype=dtype, device=device)
        return xmin, xmax
    ...
    def update_mma_step(self) -> None:
        ...
        xmma, ymma, zmma, lam, xsi, eta, mu, zet, s, low, upp = mmasub(
            ...
            self.mma_xold1,
            self.mma_xold2,
            self.f0val_mma.to(torch.float64),
            self.mma_df0dx,
            self.fval_mma.to(torch.float64),
            self.mma_dfdx,
            self.mma_low,
            self.mma_upp,
            self.mma_a0,
        ...
        self.mma_low = low
        self.mma_upp = upp
        self._evaluate_mma_candidate_(xmma)
    ...
    def shape_selection_gui(self) -> bool:
        ...
        if self.mma_kkt_norm is not None:
            psim.Text(f"mma kkt = {self.mma_kkt_norm:.3e}")
            psim.Text(f"mma residu max = {self.mma_residual_max:.3e}")
        return clicked
